    ...
    where R: io::Read
{
    ...
    #[inline(always)]
    fn read_le_f32(&mut self) -> io::Result<f32> {
        self.read_le_u32().map(|u| { f32::from_bits(u) })
    }
}
...
pub struct WavSpecSurge {
    ...
    ///original WavSpec container, without extra surge chunks
    pub specx: Option<WavSpecEx>,

    ///data parsed from the CLM chunk, potentially used by surge
    pub clm:   Option<ClmSpec>,

    ///data parsed from the UHWT chunk, potentially used by surge
    pub uhwt:  Option<UhwtSpec>,

    ///data parsed from the SRGE chunk, potentially used by surge
    pub srge:  Option<SrgeSpec>,

    ///data parsed from the SRGO chunk, potentially used by surge
    pub srgo:  Option<SrgoSpec>,

    ///data parsed from the CUE chunk, potentially used by surge
    pub cue:   Option<CueSpec>,

    ///data parsed from the SMPL chunk, potentially used by surge
...
pub fn read_until_data<R: io::Read>(mut reader: R) -> Result<(WavSpecSurge, u32)> {
    ...
    let mut surge_spec_opt: Option<WavSpecSurge> = Some(WavSpecSurge::default());

    loop {
        let header = WavReader::read_chunk_header(&mut reader)?;
        match header.kind {
            ChunkKind::Fmt => {
                let specx = WavReader::read_fmt_chunk(&mut reader, header.len)?;
                if let Some(ref mut spec) = surge_spec_opt {
                    spec.specx = Some(specx);
                }
            }
            ChunkKind::Clm => {
                let clm = WavReader::read_clm_chunk(&mut reader, header.len)?;
                if let Some(ref mut spec) = surge_spec_opt {
                    spec.clm = Some(clm);
                }
            },
            ChunkKind::Uhwt => {
                let uhwt = WavReader::read_uhwt_chunk(&mut reader, header.len);
                if let Some(ref mut spec) = surge_spec_opt {
                    spec.uhwt = Some(uhwt);
                }
            },
            ChunkKind::Srge => {
                let srge = WavReader::read_srge_chunk(&mut reader, header.len)?;
                if let Some(ref mut spec) = surge_spec_opt {
                    spec.srge = Some(srge);
                }
            },
            ChunkKind::Srgo => {
                let srgo = WavReader::read_srgo_chunk(&mut reader, header.len)?;
                if let Some(ref mut spec) = surge_spec_opt {
                    spec.srgo = Some(srgo);
                }
            },
            ChunkKind::Cue => {
                let cue = WavReader::read_cue_chunk(&mut reader, header.len)?;
                if let Some(ref mut spec) = surge_spec_opt {
                    spec.cue = Some(cue);
                }
            },
            ChunkKind::Smpl => {
                let smpl = WavReader::read_smpl_chunk(&mut reader, header.len)?;
                if let Some(ref mut spec) = surge_spec_opt {
                    spec.smpl = Some(smpl);
                }
            },
        ...
        // via one of the try! macros (edit: ? operators) that return an Err on end of file.
    }
}
    ...
    where R: io::Read
{
    ...
    fn read_clm_chunk(reader: &mut R, chunk_len: u32) -> Result<ClmSpec> {
        ...
        let _prefix = reader.read_le_u24()?;

        let c1: char = reader.read_u8()?.into();
        let c2: char = reader.read_u8()?.into();
        ...
        match (c1,c2,c3,c4) {
            ...
            _ => {
                Err(Error::FormatError("invalid clm prefix"))
            },
        }
    }

    /// Reads the uhwt chunk of the file, returns the information it provides.
    fn read_uhwt_chunk(_reader: &mut R, _chunk_len: u32) -> UhwtSpec {
    ...
    fn read_cue_chunk(reader: &mut R, chunk_len: u32) -> Result<CueSpec> {
        ...
        let num_cues  = reader.read_le_i32()?;
        parsed += 4;

        let mut chunk_starts = vec![];

        for _cue in 0..num_cues {
            for j in 0..6 {

                let d = reader.read_le_i32()?;
        ...
        if regular {
            ...
            Err(Error::FormatError("invalid CUE chunk"))
    ...
    fn read_smpl_chunk(reader: &mut R, chunk_len: u32) -> Result<SmplSpec> {
        ...
        if nloops > 1 {
            return Err(Error::FormatError("SMPL chunk, nloops > 1, not supported"));
        }
    ...
    pub fn new(mut reader: R) -> Result<WavReader<R>> {
        ...
        let (surge_spec, data_len) = read_until_data(&mut reader)?;
    ...
    pub fn samples<S: Sample>(&mut self) -> WavSamples<'_, R, S> {
        WavSamples {
            reader: self,
