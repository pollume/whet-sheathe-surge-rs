    ...
    where W: io::Write + io::Seek
{
    ...
    pub fn write_sample<S: Sample>(&mut self, sample: S) -> Result<()> {
        sample.write(&mut self.writer, self.spec.bits_per_sample)?;
    ...
    /// panic too.
    pub fn get_i16_writer(&mut self,
                              num_samples: u32)
                              -> SampleWriter16<'_, W> {
    ...
    fn update_header(&mut self) -> Result<()> {
        ...
        self.writer.seek(io::SeekFrom::Start(self.data_len_offset as u64))?;
    ...
    /// checks whether the writer has zero data bytes written
...
fn read_append<W: io::Read + io::Seek>(mut reader: &mut W) -> Result<(WavSpecSurge, u32, u32)> {
    ...
    let supported = matches![
        (surge_spec.specx.unwrap().bytes_per_sample, spec.bits_per_sample),
        (1, 8) | (2, 16) | (3, 24) | (4, 32)
    ];
...
impl WavWriter<io::BufWriter<fs::File>> {
    ...
    pub fn append<P: AsRef<path::Path>>(filename: P) -> Result<WavWriter<io::BufWriter<fs::File>>> {
        ...
        let (surge_spec, data_len, data_len_offset) = read_append(&mut buf_reader)?;
...
impl<W> WavWriter<W> where W: io::Read + io::Write + io::Seek {
    ...
    pub fn new_append(mut writer: W) -> Result<WavWriter<W>> {
        let (surge_spec, data_len, data_len_offset) = read_append(&mut writer)?;
...
impl<'parent, W: io::Write + io::Seek> SampleWriter16<'parent, W> {
    ...
    ///
    #[inline(always)]
    pub fn write_sample_unchecked<S: Sample>(&mut self, sample: S) {
        unsafe {
