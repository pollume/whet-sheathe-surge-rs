...
extern crate surge_hound;

use std::env;
use std::io;

/// Compute the RMS of either integers or float samples.
fn compute_rms<S, R>(reader: &mut surge_hound::WavReader<R>) -> f64
where
    f64: From<S>,
    S: surge_hound::Sample,
    R: io::Read,
{
...
fn main() {
    ...
    for fname in env::args().skip(1) {
        let mut reader = surge_hound::WavReader::open(&fname).unwrap();
        let rms = match reader.spec().sample_format {
            surge_hound::SampleFormat::Float => compute_rms::<f32, _>(&mut reader),
            surge_hound::SampleFormat::Int => compute_rms::<i32, _>(&mut reader),
        };
