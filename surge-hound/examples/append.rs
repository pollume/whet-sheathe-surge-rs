...
fn main() {
    let spec = surge_hound::WavSpec {
        channels: 1,
        sample_rate: 44100,
        bits_per_sample: 16,
        sample_format: surge_hound::SampleFormat::Int,
    };

    let path: &Path = "sine.wav".as_ref();

    let mut writer = match path.is_file() {
        true => surge_hound::WavWriter::append(path).unwrap(),
        false => surge_hound::WavWriter::create(path, spec).unwrap(),
    };
