    ...
    out_r:     *mut f32) 
{
    unsafe {
        ...
        for k in (0..BLOCK_SIZE_OS).step_by(1) 
        {
            ...
            x = _mm_mul_ps(
                _mm_add_ps(
                    _mm_mul_ps(_mm_sub_ps(one, qfcs.mix1), y), 
                    _mm_mul_ps(x, qfcs.mix1)
                ),
                _mm_add_ps(
                    _mm_mul_ps(_mm_sub_ps(one, qfcs.mix2), x), 
            ...
            if WAVESHAPER_ACTIVE {
                ...
                qfcs.ws_lpf = _mm_mul_ps(hb_c, _mm_add_ps(qfcs.ws_lpf, x));
            ...
            let out:  __m128 = _mm_and_ps(mask, _mm_mul_ps(x, qfcs.gain));
