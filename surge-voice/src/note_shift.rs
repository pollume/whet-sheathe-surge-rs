...
impl SurgeVoice {
    ...
    /**
      | Given a note0 and an oscilator this
      | returns the appropriate note.
      |
      | This is a pretty easy calculation in
      | non-absolute mode. Just add.
      |
      | But in absolute mode you need to find the
      | virtual note which would map to that
      | frequency shift.
      */
        ...
        match pitch_is_absolute {
            true => {
                ...
                /*
                 ...
                 |
                 | OK so now what we are searching
